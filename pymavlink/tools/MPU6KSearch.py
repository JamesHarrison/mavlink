    ...
    while True:
        ...
        if mtype == 'RAW_IMU':
            ...
            if m.Name.startswith(
                    'INS_ACCOFFS_') or m.Name.startswith('INS_ACC2OFFS_'):
                if m.Value == 0.0:
                    print(('UNCALIBRATED: %s' % m))
                    return False
        elif mtype == 'PARAM_VALUE':
            if m.param_id.startswith(
                    'INS_ACCOFFS_') or m.param_id.startswith('INS_ACC2OFFS_'):
...
for i in range(len(filelist)):
    f = filelist[i]
    print(("Checking %s ... [found=%u i=%u/%u]" %
           (f, len(found), i, len(filelist))))
    try:
        if IMUCheckFail(f):
            found.append(f)
    except Exception as e:
...
for f in found:
    ...
    if not arcname.startswith('201'):
        ...
        arcname = "%s-%s" % (time.strftime("%Y-%m-%d-%H-%M-%S",
...
print(("Created %s with %u of %u matching logs" %
       (results, len(found), len(filelist))))
