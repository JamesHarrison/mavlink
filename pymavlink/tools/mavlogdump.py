...
parser.add_argument(
    "--format",
    default=None,
    help="Change the output format between 'standard', 'json', and 'csv'. For the CSV output, you must supply types that you want.")
parser.add_argument(
    "--csv_sep",
    dest="csv_sep",
    default=",",
    help="Select the delimiter between columns for the output CSV file. Use 'tab' to specify tabs. Only applies when --format=csv")
parser.add_argument(
    "--types",
    default=None,
        ...
        for type in types:
            try:
                ...
                fields += [
                    type +
                    '.' +
                    x for x in inspect.getargspec(
                        getattr(
                            mavutil.mavlink,
                            typeClass).__init__).args[
                        1:]]
...
# Track the last timestamp value. Used for compressing data for the CSV
# output format.
last_timestamp = None
...
while True:
    ...
    if m is None:
        # FIXME: Make sure to output the last CSV message before dropping out
        # of this loop
        break
    ...
    if types is not None and m.get_type(
    ) not in types and m.get_type() != 'BAD_DATA':
        continue
    ...
    # If we're just logging, pack in the timestamp and data into the output
    # file.
    if output:
    ...
    if args.format == 'json':
        # Format our message as a Python dict, which gets us almost to proper
        # JSON format
        data = m.to_dict()
        ...
        # Also, if it's a BAD_DATA message, make it JSON-compatible by removing
        # array objects
    ...
    # CSV format outputs columnar data with a user-specified delimiter
        ...
        if timestamp == last_timestamp or last_timestamp is None:
            newData = [str(data[y.split('.')[-
                                             1]]) if y.split('.')[0] == type and y.split('.')[-
                                                                                              1] in data else "" for y in fields]
            for i, val in enumerate(newData):
                if val:
                    csv_out[i] = val

        # Otherwise if this is a new timestamp, print out the old output data,
        # and store the current message for later output.
        else:
            csv_out[0] = "{:.8f}".format(last_timestamp)
            print((args.csv_sep.join(csv_out)))
            csv_out = [str(data[y.split('.')[-
                                             1]]) if y.split('.')[0] == type and y.split('.')[-
                                                                                              1] in data else "" for y in fields]
