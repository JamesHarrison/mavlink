...
def flight_modes(logfile):
    ...
    while True:
        ...
        if (mode_start_timestamp == -1):
            ...
            time_in_mode[previous_mode] = time_in_mode[
                previous_mode] + (m._timestamp - mode_start_timestamp)
            ...
            if (seconds_per_percent == -1 and previous_percent != -1
                ...
                seconds_per_percent = (
                    m._timestamp - mode_start_timestamp) / (mlog.percent - previous_percent)
    ...
    if (seconds_per_percent != -1):
        ...
        time_in_mode[previous_mode] = time_in_mode[
            previous_mode] + seconds_remaining
        ...
        for key, value in time_in_mode.items():
            print(('%-12s %s %.2f%%' % (key,
                                        str(datetime.timedelta(seconds=int(value))),
