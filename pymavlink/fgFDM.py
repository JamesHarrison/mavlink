...
class fgFDMVariableList(object):
    ...
    def add(self, varname, arraylength=1, units=None):
        self.vars[varname] = fgFDMVariable(
            self._nextidx,
            arraylength,
            units=units)
...
class fgFDM(object):
    ...
    def __init__(self):
        ...
        self.mapping.add(
            'altitude',
            units='meters')  # above sea level (meters)
        ...
        self.mapping.add('psi', units='radians')
        self.mapping.add(
            'alpha',
            units='radians')      # angle of attack (radians)
        # side slip angle (radians)
        self.mapping.add('beta', units='radians')
        ...
        self.mapping.add('v_north', units='fps')
        # east velocity in local/body frame, fps
        self.mapping.add('v_east', units='fps')
        # down/vertical velocity in local/body frame, fps
        self.mapping.add('v_down', units='fps')
        self.mapping.add(
            'v_wind_body_north',
            units='fps')   # north velocity in local/body frame
        self.mapping.add(
            'v_wind_body_east',
            units='fps')    # east velocity in local/body frame
        self.mapping.add(
            'v_wind_body_down',
            units='fps')    # down/vertical velocity in local/body

        # Accelerations
        self.mapping.add(
            'A_X_pilot',
            units='fpss')  # X accel in body frame ft/sec^2
        self.mapping.add(
            'A_Y_pilot',
            units='fpss')  # Y accel in body frame ft/sec^2
        self.mapping.add(
            'A_Z_pilot',
            units='fpss')  # Z accel in body frame ft/sec^2
        ...
        self.mapping.add(
            'eng_state',
            self.FG_MAX_ENGINES)  # Engine state (off, cranking, running)
        self.mapping.add('rpm', self.FG_MAX_ENGINES)  # Engine RPM rev/min
        self.mapping.add(
            'fuel_flow',
            self.FG_MAX_ENGINES)  # Fuel flow gallons/hr
        ...
        self.mapping.add(
            'cht',
            self.FG_MAX_ENGINES)  # Cylinder head temp deg F
        self.mapping.add('mp_osi', self.FG_MAX_ENGINES)  # Manifold pressure
        self.mapping.add(
            'tit',
            self.FG_MAX_ENGINES)  # Turbine Inlet Temperature
        ...
        self.mapping.add('warp', units='seconds')
        # visibility in meters (for env. effects)
        self.mapping.add('visibility', units='meters')
    ...
    def get(self, varname, idx=0, units=None):
        ...
        if units:
            value = self.convert(
                value,
                self.mapping.vars[varname].units,
                units)
        return value
