...
class DFMessage(object):
    ...
    def get_msgbuf(self):
        ...
        return struct.pack("BBB",
                           0xA3,
                           0x95,
                           self.fmt.type) + struct.pack(self.fmt.msg_struct,
                                                        *values)
...
class DFReader(object):
    ...
    def _find_time_base(self):
        ...
        if self._zero_time_base:
            return
        gps1 = self.recv_match(
            type='GPS',
            condition='getattr(GPS,"Week",0)!=0 or getattr(GPS,"GPSTime",0)!=0')
        if gps1 is None:
            self._rewind()
            return
        ...
        for type in counts2:
            self.msg_rate[type] = (
                counts2[type] - counts1[type]) / float(t2 - t1)
...
class DFReader_binary(DFReader):
    ...
    def _parse_next(self):
        ...
        while (ord(hdr[0]) != self.HEAD1 or ord(hdr[1]) !=
               self.HEAD2 or ord(hdr[2]) not in self.formats):
        ...
        if skip_bytes != 0:
            if self.remaining < 528:
                return None
            print(("Skipped %u bad bytes in log %s remaining=%u" %
                   (skip_bytes, skip_type, self.remaining)))
        ...
        if name == 'FMT' and elements[0] not in self.formats:
            ...
            self.formats[
                elements[0]] = DFFormat(
                elements[0], null_term(
                    elements[2]), elements[1], null_term(
                    elements[3]), null_term(
                    elements[4]))
...
class DFReader_text(DFReader):
    ...
    def _parse_next(self):
        ...
        if name == 'FMT':
            # add to formats
            # name, len, format, headings
            self.formats[elements[2]] = DFFormat(
                int(elements[0]), elements[2], int(elements[1]), elements[3], elements[4])
