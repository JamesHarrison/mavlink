...
def mag_heading(
    ...
    headX = mag_x * cos_pitch_sq - dcm_matrix.c.x * \
        (mag_y * dcm_matrix.c.y + mag_z * dcm_matrix.c.z)
...
def mag_heading_motors(
    ...
    headX = mag_x * cos(ATTITUDE.pitch) + mag_y * sin(ATTITUDE.roll) * \
        sin(ATTITUDE.pitch) + mag_z * cos(ATTITUDE.roll) * sin(ATTITUDE.pitch)
...
def second_derivative_9(var, key):
    ...
    # http://www.holoborodko.com/pavel/numerical-methods/numerical-derivative/smooth-low-noise-differentiators/
    f = data
    ret = ((f[8] + f[0]) + 4 * (f[7] + f[1]) + 4 * (f[6] + f[2]) -
           4 * (f[5] + f[3]) - 10 * f[4]) / (64 * h**2)
    return ret
...
def roll_estimate(
    ...
    if SENSOR_OFFSETS is not None and ofs is not None:
        ...
        if mul is not None:
            ...
            rz *= mul[2]
    return lowpass(
        degrees(-asin(ry / sqrt(rx**2 + ry**2 + rz**2))), '_roll', smooth)
...
def gps_velocity_old(GPS_RAW_INT):
    '''return GPS velocity vector'''
    return Vector3(GPS_RAW_INT.vel *
                   0.01 *
                   cos(radians(GPS_RAW_INT.cog *
                               0.01)), GPS_RAW_INT.vel *
                   0.01 *
                   sin(radians(GPS_RAW_INT.cog *
                               0.01)), 0)
...
def earth_gyro(RAW_IMU, ATTITUDE):
    ...
    accel = Vector3(
        degrees(
            RAW_IMU.xgyro), degrees(
            RAW_IMU.ygyro), degrees(
                RAW_IMU.zgyro)) * 0.001
    return r * accel
...
def airspeed_energy_error(NAV_CONTROLLER_OUTPUT, VFR_HUD):
    ...
    airspeed_energy_error = (
        (target_airspeed * target_airspeed) - (aspeed_cm * aspeed_cm)) * 0.00005
...
def rover_turn_circle(SERVO_OUTPUT_RAW):
    ...
    steering_angle = max_wheel_turn * \
        (SERVO_OUTPUT_RAW.servo1_raw - 1500) / 400.0
...
def ekf1_pos(EKF1):
    ...
    if ekf_home is None:
        ...
        (ekf_home.Lat, ekf_home.Lng) = gps_offset(
            ekf_home.Lat, ekf_home.Lng, -EKF1.PE, -EKF1.PN)
