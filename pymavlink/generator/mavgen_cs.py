...
# Map of field type to bitconverter bytedecoding function, and number of
# bytes used for the encoding
mapType = {
    'float': ('ToSingle', 4),
    'double': ('ToDouble', 8),
    'int8_t': ('ToInt8', 1),
...
# Map of field names to names that are C# compatible and not illegal class
# field names
mapFieldName = {
    'fixed': '@fixed'
}
...
def generate_Deserialization(outf, messages):
    ...
    for m in messages:
        ...
        for f in m.ordered_fields:
            ...
            # mapping 'char' to byte here since there is no real equivalent in
            # the CLR
...
""")
    outf.write(
        '\t\tpublic const string WireProtocolVersion = "%s";' %
        xml[0].wire_protocol_version)
    outf.write(
        '\n\t\tpublic const byte ProtocolMarker = 0x%x;' %
        xml[0].protocol_marker)
    outf.write(
        '\n\t\tpublic const bool CrcExtra = %s;' % str(
            xml[0].crc_extra).lower())
    outf.write(
        '\n\t\tpublic const bool IsLittleEndian = %s;' % str(
            xml[0].little_endian).lower())
    ...
    for m in messages:
        classname = "Msg_%s" % m.name.lower()
        outf.write(
            "\n\t\t\t{%s, new MavPacketInfo(Deserialize_%s, %s)}," %
            (m.id, m.name, m.crc_extra))
    outf.write("\n\t\t};\n")
...
def generate(basename, xml):
    ...
    # Some build commands depend on the platform - eg MS .NET Windows Vs Mono
    # on Linux
...
[assembly: AssemblyFileVersion("1.0.0.0")]
    ...
    compileCommand = "%s %s" % (
        cscCommand, "/target:library /debug /out:" + outputLibraryPath)
    compileCommand = compileCommand + " /doc:" + \
        os.path.normpath(dir + "/mavlink.xml")
