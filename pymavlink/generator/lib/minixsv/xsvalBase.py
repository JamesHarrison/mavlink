...
class XsValBase:
    ...
    def validate(self, inputTree, xsdTreeList):
        ...
        self.inputNsPrefix = self.inputRoot.getNsPrefix(
            self.inputRoot.getNsName())
        if self.inputNsPrefix is not None:
        ...
        self.xsdLookupDict = {
            "ElementDict": wxsdLookupDict["ElementDict"].copy(),
        ...
        if inputRootNsName in self.xsdElementDict:
            ...
            try:
                self._checkElementTag(
                    self.xsdElementDict[inputRootNsName],
                    self.inputRoot,
                    (self.inputRoot,
                     ),
                    0)
            ...
            if not self.errorHandler.hasErrors():
                ...
                for idref in list(self.xsdIdRefDict.keys()):
                    if idref not in self.xsdIdDict:
                        self._addError(
                            "There is no ID/IDREF binding for IDREF %s" %
                            repr(idref),
                            self.xsdIdRefDict[idref])
    ...
    def _expandIncludes(self, baseTree, tree, includeDict, lookupDict):
        ...
        for includeNode in rootNode.getChildrenNS(namespaceURI, "include"):
            ...
            expNamespace = rootNode.getAttributeOrDefault(
                "targetNamespace",
                None)
            self._includeSchemaFile(
                baseTree,
                tree,
                includeNode,
                expNamespace,
                includeUrl,
                includeNode.getBaseUrl(),
                includeDict,
                lookupDict,
        ...
        for redefineNode in rootNode.getChildrenNS(namespaceURI, "redefine"):
            ...
            for childNode in redefineNode.getChildren():
                ...
                if childNode.getLocalName() in ("complexType", "simpleType"):
                    ...
                    self._addError(
                        "%s not allowed as child of 'redefine'!" %
                        repr(
                            childNode.getLocalName()),
                        childNode)
                    continue

                redefType = NsNameTuple(
                    (expNamespace, childNode.getAttribute("name")))
                if redefType in xsdDict:
                    orgRedefType = NsNameTuple(
                        (expNamespace, redefType[1] + "__ORG"))
...
#                        self._addError ("Duplicate component %s found within 'redefine'!" %repr(redefType), childNode)
                    ...
                    self._addError(
                        "Type %s not found in imported schema file!" %
                        (repr(redefType)),
                        childNode)

                dummy, attrNodes, attrNsNameFirst = childNode.getXPathList(
                    ".//@base | .//@ref" %
                    vars())
    ...
    def _expandImports(self, baseTree, tree, includeDict, lookupDict):
        ...
        for includeNode in rootNode.getChildrenNS(namespaceURI, "import"):
            ...
            if expNamespace == self._getTargetNamespace(includeNode):
                self._addError(
                    "Target namespace and target namespace of imported schema must not be the same!",
                    includeNode)
                continue

            includeUrl = includeNode.getAttributeOrDefault(
                "schemaLocation",
                None)
            if expNamespace is not None and includeUrl is None:
                includeUrl = expNamespace + ".xsd"
            if includeUrl is not None:
                ...
                self._addError(
                    "schemaLocation attribute for import directive missing!",
                    includeNode)
            rootNode.removeChild(includeNode)
    ...
    def _importWellknownSchemas(self, lookupDict):
        ...
        for schemaFile in (
            ...
            subTree = self._parseIncludeSchemaFile(
                wxsdTree,
                wxsdTree,
                None,
                file,
    ...
    def _includeSchemaFile(
        ...
        if rootNode.getNsName() != stRootNode.getNsName():
            self._raiseError(
                "Root tag of file %s does not match!" %
                repr(includeUrl),
                nextSibling)
        ...
        if stRootNode.hasAttribute("targetNamespace"):
            if expNamespace != stRootNode["targetNamespace"]:
                self._raiseError(
                    "Target namespace of file %s does not match!" %
                    repr(includeUrl),
                    nextSibling)
        else:
            if expNamespace is not None:
        ...
        if includeUrl not in (
                r"C:\Program Files\Python24\Lib\site-packages\minixsv\xml.xsd",
            ...
            rootNode.insertSubtree(
                nextSibling,
                subTree,
                insertSubTreeRootNode=0)
    ...
    def _parseIncludeSchemaFile(
        ...
        try:
            ...
            # FIXME: sometimes an URLError is catched instead of a standard
            # IOError
            try:
                dummy = e.errno
            except:
            ...
            if e.errno in (
                    2,
                    "socket error",
                    "url error"):  # catch IOError: No such file or directory
                self._raiseError(
                    "%s: '%s'" %
                    (e.strerror, e.filename), nextSibling)
            else:
                raise
    ...
    def _updateLookupTables(self, rootNode, lookupDict):
        ...
        for localName, lookupDictName in list(schemaTagDict.items()):
            for node in rootNode.getChildrenNS(XSD_NAMESPACE, localName):
                ...
                if (targetNamespace, node.getAttribute("name")) not in lookupDict[
                        lookupDictName]:
                    lookupDict[lookupDictName][
                        (targetNamespace, node.getAttribute("name"))] = node

        # retrieve all identity constraints
        for identConstrTagName in ("unique", "key", "keyref"):
            identConstrNodeList = rootNode.getElementsByTagNameNS(
                XSD_NAMESPACE,
                identConstrTagName)
            for identConstrNode in identConstrNodeList:
                targetNamespace = self._getTargetNamespace(identConstrNode)
                identConstrNsLocalName = NsNameTupleFactory(
                    (targetNamespace, identConstrNode.getAttribute("name")))
                if identConstrNsLocalName not in lookupDict[
                        "IdentityConstrDict"]:
                    lookupDict["IdentityConstrDict"][identConstrNsLocalName] = {
                        "Node": identConstrNode,
                        "ValueDict": {}}
...
#                                    %(repr(identConstrNsLocalName)), identConstrNode)
    ...
    def _checkComplexTypeTag(
        ...
        complexContentNode = xsdNode.getFirstChildNS(
            self.xsdNsURI,
            "complexContent")
        simpleContentNode = xsdNode.getFirstChildNS(
            self.xsdNsURI,
            "simpleContent")
        if complexContentNode is not None:
            inputChildIndex, baseTypeAttributes = self._checkComplexContentTag(
                xsdParentNode, complexContentNode, inputNode, inputChildIndex, usedAsBaseType, baseTypeAttributes)
        elif simpleContentNode is not None:
            inputChildIndex, baseTypeAttributes = self._checkSimpleContentTag(
                xsdParentNode, simpleContentNode, inputNode, inputChildIndex, usedAsBaseType, baseTypeAttributes)
        else:
            inputChildIndex, baseTypeAttributes = self._checkComplexTypeContent(
                xsdParentNode, xsdNode, inputNode, inputChildIndex, usedAsBaseType, baseTypeAttributes)
    ...
    def _checkComplexContentTag(
        ...
        if extensionNode is not None:
            inputChildIndex, baseTypeAttributes = self._checkExtensionComplexContent(
                xsdParentNode, extensionNode, inputNode, inputChildIndex, usedAsBaseType, baseTypeAttributes)
        else:
            restrictionNode = xsdNode.getFirstChildNS(
                self.xsdNsURI,
                "restriction")
            if restrictionNode is not None:
                inputChildIndex, baseTypeAttributes = self._checkRestrictionComplexContent(
                    xsdParentNode, restrictionNode, inputNode, inputChildIndex, usedAsBaseType, baseTypeAttributes)
            else:
...
#            self._checkMixed (xsdNode, inputNode)
    ...
    def _checkSimpleContentTag(
        ...
        if inputNode.getAttribute((XSI_NAMESPACE, "nil")) == "true":
            if inputNode.getChildren() != [] or collapseString(
                    inputNode.getElementValue()) != "":
                self._addError(
                    "Element must be empty (xsi:nil='true')(1)!",
                    inputNode,
                    0)

        extensionNode = xsdNode.getFirstChildNS(self.xsdNsURI, "extension")
        if extensionNode is not None:
            inputChildIndex, baseTypeAttributes = self._checkExtensionSimpleContent(
                xsdParentNode, extensionNode, inputNode, inputChildIndex, usedAsBaseType, baseTypeAttributes)
        else:
            restrictionNode = xsdNode.getFirstChildNS(
                self.xsdNsURI,
                "restriction")
            if restrictionNode is not None:
                inputChildIndex, baseTypeAttributes = self._checkRestrictionSimpleContent(
                    xsdParentNode, xsdNode, restrictionNode, inputNode, inputChildIndex, usedAsBaseType, baseTypeAttributes)
        return inputChildIndex, baseTypeAttributes
    ...
    def _checkExtensionComplexContent(
        ...
        inputChildIndex, baseTypeAttributes = self._checkComplexTypeTag(
            xsdParentNode, self.xsdTypeDict[baseNsName], inputNode, inputChildIndex, extUsedAsBaseType)
    ...
    def _checkExtensionSimpleContent(
            ...
            usedAsBaseType,
            baseTypeAttributes):
        self._checkSimpleType(
            xsdNode,
            "base",
            inputNode,
            inputNode.getTagName(),
            inputNode.getElementValue(),
            None,
            checkAttribute=0)
        if xsdNode.hasAttribute("BaseTypes"):
            xsdParentNode["BaseTypes"] = xsdNode["BaseTypes"]
        inputChildIndex, baseTypeAttributes = self._checkSimpleTypeContent(
            xsdParentNode, xsdNode, inputNode, inputChildIndex, usedAsBaseType, baseTypeAttributes)
        return inputChildIndex, baseTypeAttributes
    ...
    def _checkRestrictionComplexContent(
            ...
            usedAsBaseType,
            baseTypeAttributes):
        # first check against base type (retrieve only the base type
        # attributes)
        baseNsName = xsdNode.getQNameAttribute("base")
        inputChildIndex, baseTypeAttributes = self._checkComplexTypeTag(
            xsdParentNode, self.xsdTypeDict[baseNsName], inputNode, inputChildIndex, "restriction")
    ...
    def _checkRestrictionSimpleContent(
        ...
        try:
            ...
            self.simpleTypeVal.checkSimpleTypeDef(
                inputNode,
                simpleContentNode,
                inputNode.getTagName(),
                inputNode.getElementValue(),
                ...
                idCheck=1)
            xsdNode["BaseTypes"] = string.join(
                simpleTypeReturnDict["BaseTypes"],
                " ")
    ...
    def _checkComplexTypeContent(
        ...
        if inputNode.getAttribute((XSI_NAMESPACE, "nil")) == "true":
            if inputNode.getChildren() != [] or collapseString(
                    inputNode.getElementValue()) != "":
                self._addError(
                    "Element must be empty (xsi:nil='true')(2)!",
                    inputNode,
                    0)
        else:
            ...
            if usedAsBaseType in (None, "extension"):
                ...
                for validChildTag in validChildTags:
                    if validChildTag.getLocalName() not in (
                            "attribute",
                            "attributeGroup",
                            "anyAttribute"):
                        inputChildIndex = self._checkParticle(
                            validChildTag,
                            inputNode,
                            childTags,
                            inputChildIndex)
    ...
    def _checkSimpleTypeContent(
        ...
        if inputNode.getChildren() != []:
            raise TagException(
                "No child tags are allowed for element %s!" %
                repr(
                    inputNode.getNsName()),
                inputNode)
    ...
    def _checkMixed(self, xsdParentNode, xsdNode, inputNode):
        if xsdNode.getAttributeOrDefault("mixed", "false") == "false":
            if not collapseString(inputNode.getElementValue()) in ("", " "):
                self._addError(
                    "Mixed content not allowed for %s!" %
                    repr(
                        inputNode.getTagName()),
                    inputNode)
        else:  # mixed = true
    ...
    def _checkList(
            ...
            inputNodeList,
            currIndex):
        minOccurs = string.atoi(
            xsdNode.getAttributeOrDefault(
                "minOccurs",
                "1"))
        ...
        if occurs == 0 and minOccurs > 0:
            ...
            if expInputTagName is None:
                expInputTagName = xsdNode.getQNameAttribute("ref")
            raise TagException(
                "minOccurs (%d) of child tag %s in tag %s not available (only %d)!" %
                (minOccurs,
                 repr(expInputTagName),
                    repr(
                     inputParentNode.getTagName()),
                    occurs),
                inputParentNode,
                1)
    ...
    def _checkElementTag(
        ...
        if xsdNode.hasAttribute("ref"):
            ...
            currIndex = self._checkElementTag(
                self.xsdElementDict[refNsName],
                inputParentNode,
                inputNodeList,
                currIndex)
        else:
                ...
                for inputChild in inputNode.getChildren():
                    try:
                        if inputChild.getNsName() in self.xsdElementDict:
                            self._checkElementTag(
                                self.xsdElementDict[
                                    inputChild.getNsName()], inputNode, (inputChild,), 0)
                    except TagException as errInst:
                        self._addError(
                            errInst.errstr,
                            errInst.node,
            ...
            complexTypeNode = xsdNode.getFirstChildNS(
                self.xsdNsURI,
                "complexType")
            if not inputNode.hasAttribute((XSI_NAMESPACE, "type")):
                ...
                typeNsName = inputNode.getQNameAttribute(
                    (XSI_NAMESPACE, "type"))
                if typeNsName not in self.xsdTypeDict:
                    self._addError(
                        "Unknown overloaded type %s!" %
                        (repr(typeNsName)),
                        inputNode,
                        0)
                    return currIndex
            ...
            if complexTypeNode is not None:
                try:
                    self._checkComplexTypeTag(
                        xsdNode,
                        complexTypeNode,
                        inputNode,
                        0)
    ...
    def _checkElementValue(self, xsdNode, xsdTypeAttr, inputNode):
        ...
        if inputNode.getAttribute(
            (XSI_NAMESPACE,
             "nil")) == "true" and fixedValue is not None:
            self._addError(
                "There must be no fixed value for Element because xsi:nil='true' is specified!",
                inputNode)

        if inputNode.getAttribute(
            (XSI_NAMESPACE,
             "nil")) != "true" and inputNode.getElementValue() == "":
    ...
    def _checkSimpleType(
        ...
        if checkAttribute == 0 and inputNode.hasAttribute(
                (XSI_NAMESPACE, "nil")):
            if (inputNode[(XSI_NAMESPACE, "nil")] == "true" and
                    collapseString(inputNode.getElementValue()) != ""):
                self._addError(
                    "Element must be empty (xsi:nil='true')(3)!",
                    inputNode,
                    0)
            return retValue
        ...
        try:
            ...
            simpleTypeNode = xsdNode.getFirstChildNS(
                self.xsdNsURI,
                "simpleType")
            if simpleTypeNode is not None:
                self.simpleTypeVal.checkSimpleTypeDef(
                    inputNode,
                    simpleTypeNode,
                    attrName,
                ...
                self._addError(
                    "Attribute %s requires a simple or mixed type!" %
                    repr(attrName),
                    inputNode)
            else:
                typeNsName = xsdNode.getQNameAttribute(xsdTypeAttr)
                if typeNsName != (None, None):
                    self.simpleTypeVal.checkSimpleType(
                        inputNode,
                        attrName,
                        typeNsName,
                        attrValue,
            ...
            xsdNode["BaseTypes"] = string.join(
                simpleTypeReturnDict["BaseTypes"],
                " ")
            xsdNode["primitiveType"] = str(
                simpleTypeReturnDict["primitiveType"])

            retValue = simpleTypeReturnDict
            if "wsAction" in simpleTypeReturnDict:
                if checkAttribute:
                    attrValue = inputNode.processWsAttribute(
                        attrName,
                        simpleTypeReturnDict["wsAction"])
                else:
                    attrValue = inputNode.processWsElementValue(
                        simpleTypeReturnDict["wsAction"])
            ...
            if fixedValue is not None:
                ...
                if attrValue != fixedValue:
                    if checkAttribute:
                        self._addError(
                            "Attribute %s must have fixed value %s!" %
                            (repr(attrName), repr(fixedValue)), inputNode)
                    else:
                        self._addError(
                            "Element must have fixed value %s!" %
                            repr(fixedValue),
                            inputNode)
        ...
        for xsdChildNode in xsdNode.getChildren():
            ...
            if not childFound and exceptionRaised:
                if currIndex < len(inputNodeList):
                    ...
                    endTag = 1
                raise TagException(
                    "No suitable child tag for choice found!",
                    currNode,
                    endTag)
    ...
    def _checkGroupTag(
        ...
        if xsdNode.hasAttribute("ref"):
            refNsName = xsdNode.getQNameAttribute("ref")
            currIndex = self._checkGroupTag(
                self.xsdGroupDict[refNsName],
                inputParentNode,
                inputNodeList,
                currIndex)
        else:
    ...
    def _checkAllTag(self, xsdNode, inputParentNode, inputNodeList, currIndex):
        ...
        while (
            currIndex < len(inputNodeList)) and (
            0 in list(
                xsdChildDict.values())):
            currNode = inputNodeList[currIndex]
            for xsdChildNode in list(xsdChildDict.keys()):
                ...
                if xsdChildDict[xsdChildNode] == 0:
                    ...
                    raise TagException(
                        "Ambiguous child tag %s found in all-group!" %
                        repr(
                            currNode.getTagName()),
                        currNode)
            else:
                raise TagException(
                    "Unexpected child tag %s for all-group found!" %
                    repr(
                        currNode.getTagName()),
                    currNode)

        for xsdChildNode, occurs in list(xsdChildDict.items()):
            if xsdChildNode.getAttributeOrDefault(
                    "minOccurs",
                    "1") != "0" and occurs == 0:
                raise TagException(
                    "Child tag %s missing in all-group (%s)" %
                    (repr(
                        xsdChildNode.getAttribute("name")), repr(
                        inputParentNode.getTagName())), inputParentNode)
    ...
    def _checkAnyTag(self, xsdNode, inputParentNode, inputNodeList, currIndex):
        if currIndex >= len(inputNodeList):
            raise TagException(
                "Missing child tag (anyTag) in tag %s!" %
                repr(
                    inputParentNode.getTagName()),
                inputParentNode,
                1)
    ...
    def _checkParticle(
        ...
        if xsdTagName == "element":
            ...
            self._addError(
                "Internal error: Invalid tag %s found!" %
                repr(xsdTagName))
        return currIndex
    ...
    def _checkAttributeTags(
        ...
        inputAttrDict = {}
        for iAttrName, iAttrValue in list(
                inputNode.getAttributeDict().items()):
        ...
        for qAttrName, validAttrEntry in list(validAttrDict.items()):
            ...
            if qAttrName in self.xsdAttributeDict and self.xsdAttributeDict[
                    qAttrName] == attrRefNode:
                attributeForm = "qualified"
            else:
                attributeForm = attrRefNode.getAttributeOrDefault(
                    "form",
                    self._getAttributeFormDefault(xsdNode))
            attrRefNode.setAttribute("form", attributeForm)
            self._checkAttributeTag(
                qAttrName,
                validAttrEntry["Node"],
                attrRefNode,
                inputNode,
        ...
        for inputAttribute in list(inputAttrDict.keys()):
            if inputAttribute == (XSI_NAMESPACE, "type"):
                ...
                if parentNode.getAttributeOrDefault(
                        "nillable",
                        "false") == "false":
                    self._addError(
                        "Tag %s hasn't been defined as nillable!" %
                        repr(
                            inputNode.getTagName()),
                        inputNode)
                ...
                try:
                    ...
                    if inputAttribute[0] == None and xsdNode.getAttribute(
                            "form") == "unqualified":
                        # TODO: Check: If only local namespace is allowed, do
                        # not use target namespace???
                        if xsdNode.getAttribute("namespace") != "##local":
                            inputNamespace = self._getTargetNamespace(xsdNode)
                    self._checkWildcardAttribute(
                        xsdNode,
                        inputNode,
                        inputAttribute,
                        inputNamespace,
                    ...
                    self._addError(
                        "Unexpected attribute %s in Tag %s!" %
                        (repr(inputAttribute),
                         repr(
                            inputNode.getTagName())),
                        inputNode)
    ...
    def _checkAttributeTag(
        ...
        if qAttrName[0] == targetNamespace and xsdAttrRefNode.getAttribute(
                "form") == "unqualified":
    ...
    def _updateAttributeDict(
        ...
        if recursionKeys is None:
            recursionKeys = {}
        validAttributeNodes = xsdNode.getChildrenNS(self.xsdNsURI, "attribute")
        for validAttrGroup in xsdNode.getChildrenNS(
                self.xsdNsURI,
                "attributeGroup"):
            ...
            if refNsName in self.xsdAttrGroupDict:
                if refNsName in recursionKeys:
                    self._addError(
                        "Circular definition for attribute group %s detected!" %
                        (repr(refNsName)),
                        validAttrGroup)
                    continue
                recursionKeys[refNsName] = 1
                self._updateAttributeDict(
                    self.xsdAttrGroupDict[refNsName],
                    validAttrDict,
                    checkForDuplicateAttr,
                    recursionKeys)
        ...
        for validAttributeNode in validAttributeNodes:
            if validAttributeNode.hasAttribute("ref"):
                ...
                attrKey = (
                    self._getTargetNamespace(validAttributeNode),
                    validAttributeNode.getAttribute("name"))
                attributeRefNode = validAttributeNode

            if checkForDuplicateAttr and attrKey in validAttrDict:
                self._addError(
                    "Duplicate attribute %s found!" %
                    repr(attrKey),
                    validAttributeNode)
            else:
                validAttrDict[attrKey] = {
                    "Node": validAttributeNode,
                    "RefNode": attributeRefNode}

        anyAttributeNode = xsdNode.getFirstChildNS(
            self.xsdNsURI,
            "anyAttribute")
        if anyAttributeNode is not None:
            validAttrDict["__ANY_ATTRIBUTE__"] = {
                "Node": anyAttributeNode,
                "RefNode": anyAttributeNode}

    ########################################
    # validate wildcard specification of anyElement
    #
    def _checkWildcardElement(self, xsdNode, inputNode, inputNamespace):
        processContents = xsdNode.getAttributeOrDefault(
            "processContents",
            "lax")
        ...
        if processContents == "skip":
            ...
            if inputNode.hasAttribute((XSI_NAMESPACE, "type")):
                ...
                self._checkElementTag(
                    self.xsdElementDict[inputNsName], None, (inputNode,), 0)

            elif processContents == "strict":
                self._addError(
                    "Element definition %s not found in schema file!" %
                    repr(inputNsName),
                    inputNode)
    ...
    def _checkWildcardAttribute(
        ...
        if processContents == "skip":
            ...
            if qAttrName in self.xsdAttributeDict:
                ...
                self._addError(
                    "Attribute definition %s not found in schema file!" %
                    repr(qAttrName),
                    inputNode)
    ...
    def _checkInputNamespace(self, xsdNode, inputNode, inputNamespace):
        ...
        if namespaces == "##any":
            ...
            if inputNamespace == targetNamespace or inputNamespace is None:
                raise TagException(
                    "Node or attribute must not be part of target namespace or local!",
                    inputNode)
        else:
            for namespace in string.split(collapseString(namespaces), " "):
                if namespace == "##local" and inputNamespace is None:
                    break
                ...
                raise TagException(
                    "Node or attribute is not part of namespace %s!" %
                    repr(namespaces),
                    inputNode)
    ...
    def _checkIdentityConstraint(self, identityConstrNode, inputNode):
        ...
        identConstrNsLocalName = (
            self._getTargetNamespace(identityConstrNode),
            identConstrName)
        selectorXPathNode = identityConstrNode.getFirstChildNS(
            self.xsdNsURI,
            "selector")
        selectorNodeList, dummy, dummy = self._getXPath(
            inputNode, selectorXPathNode)

        valueDict = {}
        for selectorNode in selectorNodeList:
            fieldXPathNodeList = identityConstrNode.getChildrenNS(
                self.xsdNsURI,
                "field")
            keyValue = []
            baseTypesList = []
            for fieldXPathNode in fieldXPathNodeList:
                fieldChildList, attrNodeList, attrName = self._getXPath(
                    selectorNode, fieldXPathNode, identConstrTag)
                if len(fieldChildList) > 1:
                    self._addError(
                        "The field xPath %s of %s %s must evaluate to exactly 0 or 1 node!" %
                        (repr(
                            fieldXPathNode["xpath"]),
                            repr(identConstrTag),
                            repr(identConstrName)),
                        selectorNode)
                    return
                ...
                for fieldChild in fieldChildList:
                    if attrNodeList == []:
                        ...
                        try:
                            baseTypes = self._setBaseTypes(
                                fieldChild.getXsdNode())
                        except:
                        ...
                        try:
                            baseTypes = self._setBaseTypes(
                                attrNodeList[0].getXsdAttrNode(attrName))
                        except:
                            baseTypes = ((XSD_NAMESPACE, "anyType"),)
                        value = fieldChild
                    if baseTypes is not None:
                        if baseTypes[0] in (
                                (XSD_NAMESPACE, "anyType"), (XSD_NAMESPACE, "anySimpleType")):
                            overloadedType = inputChild.getQNameAttribute(
                                (XSI_NAMESPACE, "type"))
                            if overloadedType != (None, None):
                                baseTypes = [
                                    inputChild.getQNameAttribute(
                                        (XSI_NAMESPACE, "type")), ]
                    else:
                        self._addError(
                            "Identity constraint does not have a simple type!",
                            inputChild)
                        continue
            ...
            if keyValue != []:
                ...
                if keyValue not in valueDict:
                    ...
                    self.xsdIdentityConstrDict[identConstrNsLocalName][
                        "ValueDict"][keyValue] = baseTypesList
                else:
                    if len(keyValue) == 1:
                        self._addError(
                            "Duplicate identity constraint values %s found for identity contraint %s!" %
                            (repr(
                                keyValue[0]),
                                repr(identConstrName)),
                            selectorNode)
                        ...
                        self._addError(
                            "Duplicate identity constraint values %s found for identity contraint %s!" %
                            (repr(keyValue), repr(identConstrName)), selectorNode)
    ...
    def _checkKeyRefConstraint(self, keyrefNode, inputNode):
        ...
        selectorXPathNode = keyrefNode.getFirstChildNS(
            self.xsdNsURI,
            "selector")
        selectorNodeList, dummy, dummy = self._getXPath(
            inputNode, selectorXPathNode)
        for selectorNode in selectorNodeList:
            fieldXPathNodeList = keyrefNode.getChildrenNS(
                self.xsdNsURI,
                "field")
            keyValue = []
            for fieldXPathNode in fieldXPathNodeList:
                fieldChildList, attrNodeList, attrName = self._getXPath(
                    selectorNode, fieldXPathNode, "keyref")
                if len(fieldChildList) > 1:
                    self._addError(
                        "The field xPath of keyref %s must evaluate to exactly 0 or 1 node!" %
                        repr(keyRefName),
                        fieldXPathNode)
                    return
            ...
            keyValue = tuple(keyValue)
            if keyValue != ():
                if keyValue not in self.xsdIdentityConstrDict[
                        keyReference]["ValueDict"]:
                    self._addError(
                        "Key reference value %s is undefined for key type %s!" %
                        (repr(keyValue), repr(keyReference)), selectorNode)
                else:
                    baseTypesList = self.xsdIdentityConstrDict[
                        keyReference]["ValueDict"][keyValue]
                    for fieldXPathNode, baseTypes in zip(
                            fieldXPathNodeList, baseTypesList):
                        fieldChildList, attrNodeList, attrName = self._getXPath(
                            selectorNode, fieldXPathNode, "keyref")
                        if attrNodeList == []:
                            inputChild = fieldChildList[0]
                            refBaseTypes = self._setBaseTypes(
                                fieldChildList[0].getXsdNode())
                        else:
                            inputChild = attrNodeList[0]
                            refBaseTypes = self._setBaseTypes(
                                inputChild.getXsdAttrNode(attrName))
                        if baseTypes[0] not in refBaseTypes and refBaseTypes[
                                0] not in baseTypes:
                            if baseTypes[0] != (
                                    XSD_NAMESPACE,
                                    "anyType") and refBaseTypes[0] != (
                                    XSD_NAMESPACE,
                                    "anyType"):
                                self._addError(
                                    "Key type and key reference type does not match (%s != %s)!" %
                                    (repr(
                                        baseTypes[0]), repr(
                                        refBaseTypes[0])), inputChild)
    ...
    def _checkInputElementForm(self, xsdNode, xsdNodeNameAttr, inputNode):
        ...
        if nsNameAttr in self.xsdElementDict and self.xsdElementDict[
                nsNameAttr] == xsdNode:
            elementForm = "qualified"
        else:
            elementForm = xsdNode.getAttributeOrDefault(
                "form",
                self._getElementFormDefault(xsdNode))
        if elementForm == "qualified":
            if inputNode.getNamespaceURI() == None:
                if targetNamespace is not None:
                    self._addError(
                        "Element %s must be qualified!" %
                        repr(xsdNodeNameAttr),
                        inputNode)
            elif inputNode.getNamespaceURI() != targetNamespace:
                self._addError(
                    "%s undefined in specified namespace!" %
                    repr(xsdNodeNameAttr),
                    inputNode)
        elif elementForm == "unqualified" and inputNode.getNamespaceURI() != None:
            self._addError(
                "Local element %s must be unqualified!" %
                repr(xsdNodeNameAttr),
                inputNode)
    ...
    def _getXPath(self, node, xPathNode, identityConstraint=None):
        ...
        try:
            ...
            childList, attrNodeList, attrName = node.getXPathList(
                xPath, namespaceRef=xPathNode, useDefaultNs=0, attrIgnoreList=attrIgnoreList)
        ...
        if childList == []:
            if identityConstraint == "key":
                self.errorHandler.addError(
                    "Key is missing! XPath = %s!" %
                    repr(xPath), node)
            elif identityConstraint in ("unique", "keyref"):
                self.errorHandler.addWarning(
                    "Identity constraint is missing! XPath = %s!" %
                    repr(xPath), node)
    ...
    def _setBaseTypes(self, xsdNode):
        if xsdNode.getAttribute("BaseTypes") != None:
            ...
            baseTypeList = [
                NsNameTupleFactory(basetype) for basetype in baseTypes]
            if baseTypeList != []:
                return baseTypeList
            else:
                return None
    ...
    def _getElementFormDefault(self, node):
        schemaRootNode = node.getSchemaRootNode()
        return schemaRootNode.getAttributeOrDefault(
            "elementFormDefault",
            "unqualified")
