...
_localFacetDict = {(XSD_NAMESPACE,
                   ...
                   (XSD_NAMESPACE,
                    "union"): ("enumeration",
                               ...
                               "whiteSpace"),
                   (XSD_NAMESPACE,
                    "anySimpleType"): ("whiteSpace"),
                   }
###########################################################
# Derived validator class for validating one input schema file against the
# XML rules file
...
class XsValSchema (XsValBase):
    ...
    def _checkSchemaSecondLevel(self):
        ...
        if targetNamespace == "":
            self.errorHandler.raiseError(
                "Empty string not allowed for target namespace!",
                self.inputRoot)
    ...
    def _checkElementNodesSecondLevel(self):
        elementNodes = self.inputRoot.getElementsByTagNameNS(
            self.inputNsURI,
            "element")
        for elementNode in elementNodes:
            if not elementNode.hasAttribute(
                    "name") and not elementNode.hasAttribute("ref"):
                self._addError(
                    "Element must have 'name' or 'ref' attribute!",
                    elementNode)
                continue
            ...
            if elementNode.hasAttribute("ref"):
                for attrName in ("name", "type", "form"):
                    if elementNode.hasAttribute(attrName):
                        self._addError(
                            "Element with 'ref' attribute must not have %s attribute!" %
                            repr(attrName),
                            elementNode)
                        continue

            complexTypeNode = elementNode.getFirstChildNS(
                self.inputNsURI,
                "complexType")
            simpleTypeNode = elementNode.getFirstChildNS(
                self.inputNsURI,
                "simpleType")
            if elementNode.hasAttribute("ref") and (
                    complexTypeNode is not None or simpleTypeNode is not None):
                self._addError(
                    "Element with 'ref' attribute must not have type definition!",
                    elementNode)
                continue
            if elementNode.hasAttribute("type") and (
                    complexTypeNode is not None or simpleTypeNode is not None):
                self._addError(
                    "Element with 'type' attribute must not have type definition!",
                    elementNode)
                continue
    ...
    def _checkNotationNodesSecondLevel(self):
        notationNodes = self.inputRoot.getElementsByTagNameNS(
            self.inputNsURI,
            "notation")
        for notationNode in notationNodes:
            if not notationNode.hasAttribute(
                    "public") and not notationNode.hasAttribute("system"):
                self._addError(
    ...
    def _checkAnyNodesSecondLevel(self):
        anyNodes = self.inputRoot.getElementsByTagNameNS(
            self.inputNsURI,
            "any")
    ...
    def _checkGroupNodesSecondLevel(self):
        groupNodes = self.inputRoot.getElementsByTagNameNS(
            self.inputNsURI,
            "group")
...
#                self._checkGroupNodeCircularDef(groupNode, {groupNode["name"]:1})
    ...
    def _checkGroupNodeCircularDef(self, groupNode, groupNameDict):
        ...
        for childGroupRefNode in childGroupsRefNodes:
            if childGroupRefNode.hasAttribute("ref"):
                childGroupNode = self.xsdGroupDict[
                    childGroupRefNode.getQNameAttribute("ref")]
                if childGroupNode["name"] not in groupNameDict:
                    groupNameDict[childGroupNode["name"]] = 1
                    self._checkGroupNodeCircularDef(
                        childGroupNode,
                        groupNameDict)
                else:
                    self._addError(
                        "Circular definition of group %s!" %
                        repr(
                            childGroupNode["name"]),
                        childGroupNode)
    ...
    def _checkAttrGroupNodesSecondLevel(self):
        attributeGroupNodes = self.inputRoot.getElementsByTagNameNS(
            self.inputNsURI,
            "attributeGroup")
    ...
    def _checkAttributeNodesSecondLevel(self):
        attributeNodes = self.inputRoot.getElementsByTagNameNS(
            XSD_NAMESPACE,
            "attribute")
        for attributeNode in attributeNodes:
            if os.path.basename(
                    attributeNode.getFilePath()) != "XMLSchema-instance.xsd":
                ...
                if (attributeNode.getParentNode() == self.inputRoot or
                    ...
                    if self._getTargetNamespace(
                            attributeNode) == XSI_NAMESPACE:
                        self._addError(
                            "Target namespace of an attribute must not match '%s'!" %
                            XSI_NAMESPACE,
                            attributeNode)

            if not attributeNode.hasAttribute(
                    "name") and not attributeNode.hasAttribute("ref"):
                self._addError(
                    "Attribute must have 'name' or 'ref' attribute!",
                    attributeNode)
                continue

            if attributeNode.getAttribute("name") == "xmlns":
                self._addError(
                    "Attribute must not match 'xmlns'!",
                    attributeNode)

            if attributeNode.hasAttribute("ref"):
                if attributeNode.hasAttribute("name"):
                    self._addError(
                        "Attribute may have 'name' OR 'ref' attribute!",
                        attributeNode)
                if attributeNode.hasAttribute("type"):
                    self._addError(
                        "Attribute may have 'type' OR 'ref' attribute!",
                        attributeNode)
                if attributeNode.hasAttribute("form"):
                    self._addError(
                        "Attribute 'form' is not allowed in this context!",
                        attributeNode)

                if attributeNode.getFirstChildNS(
                        XSD_NAMESPACE,
                        "simpleType") != None:
                    self._addError(
                        "Attribute may only have 'ref' attribute OR 'simpleType' child!",
                        attributeNode)
            ...
            if attributeNode.hasAttribute("type"):
                if attributeNode.getFirstChildNS(
                        ...
                        "simpleType") != None:
                    self._addError(
                        "Attribute may only have 'type' attribute OR 'simpleType' child!",
                        attributeNode)
            ...
            if use in (
                    "required",
                    "prohibited") and attributeNode.hasAttribute("default"):
                self._addError(
                    "Attribute 'default' is not allowed, because 'use' is '%s'!" %
                    (use),
                    attributeNode)
    ...
    def _checkComplexTypesSecondLevel(self):
        ...
        contentNodes, dummy, dummy = self.inputRoot.getXPathList(
            ".//%(prefix)scomplexContent/%(prefix)srestriction | .//%(prefix)scomplexContent/%(prefix)sextension" %
            vars())
        ...
        contentNodes, dummy, dummy = self.inputRoot.getXPathList(
            ".//%(prefix)ssimpleContent/%(prefix)srestriction | .//%(prefix)ssimpleContent/%(prefix)sextension" %
            vars())
        for contentNode in contentNodes:
            ...
            if baseNsName != (XSD_NAMESPACE, "anyType"):
                ...
                self._addError(
                    "Referred type must not be 'anyType'!",
                    contentNode)
            # check for unique ID
            self._checkNodeId(contentNode)

        complexTypeNodes, dummy, dummy = self.inputRoot.getXPathList(
            ".//%(prefix)scomplexType | .//%(prefix)sextension" %
            vars())
            ...
            for key, val in list(validAttrDict.items()):
                ...
                if attrType == (XSD_NAMESPACE, "ID"):
                    if not idAttrNode:
                        ...
                        # TODO: check also if attribute has a type which is
                        # derived from ID!
                        self._addError(
                            "Two attribute declarations of complex type are IDs!",
                            val["Node"])
        ...
        contentNodes, dummy, dummy = self.inputRoot.getXPathList(
            ".//%(prefix)scomplexType/%(prefix)s*" %
            vars())
        for contentNode in contentNodes:
            self._checkOccurs(contentNode)

        contentNodes, dummy, dummy = self.inputRoot.getXPathList(
            ".//%(prefix)scomplexContent | .//%(prefix)ssimpleContent" %
            vars())
        for contentNode in contentNodes:
            # check for unique ID
            self._checkNodeId(contentNode)
    ...
    def _checkParticlesSecondLevel(self):
        ...
        particleNodes, dummy, dummy = self.inputRoot.getXPathList(
            ".//%(prefix)sall | .//%(prefix)schoice | .//%(prefix)ssequence" %
            vars())
        for particleNode in particleNodes:
            elementTypeDict = {}
            elementNameDict = {}
            groupNameDict = {}
            self._checkContainedElements(
                particleNode,
                particleNode.getLocalName(),
                elementNameDict,
                elementTypeDict,
                groupNameDict)
        ...
        for childNode in node.getChildren():
            ...
            if childParticleType in ("sequence", "choice", "all"):
                ...
                if childNode["ref"] != None:
                    childGroupNode = self.xsdGroupDict[
                        childNode.getQNameAttribute("ref")]
                    if childGroupNode["name"] not in groupNameDict:
                        groupNameDict[childGroupNode["name"]] = 1
                ...
                if childNode.getLocalName() == "any":
                    ...
                    elementName = childNode.getAttributeOrDefault(
                        "name",
                ...
                if childNode.hasAttribute("type"):
                    if elementName not in elementTypeDict:
                        ...
                        self._addError(
                            "Element %s has identical name and different types within %s!" %
                            (repr(elementName), repr(particleType)), childNode)
                if particleType != "sequence":
                    if elementName not in elementNameDict:
                        elementNameDict[elementName] = 1
                    else:
                        self._addError(
                            "Element %s is not unique within %s!" %
                            (repr(elementName), repr(particleType)), childNode)
    ...
    def _checkSimpleTypesSecondLevel(self):
        ...
        simpleTypeNodes, dummy, dummy = self.inputRoot.getXPathList(
            ".//%(prefix)ssimpleType" %
            vars())
        ...
        restrictionNodes, dummy, dummy = self.inputRoot.getXPathList(
            ".//%(prefix)ssimpleType/%(prefix)srestriction" %
            vars())
        for restrictionNode in restrictionNodes:
            ...
            if not restrictionNode.hasAttribute("base") and restrictionNode.getFirstChildNS(
                    self.inputNsURI,
                    "simpleType") == None:
                self._addError(
                    "Simple type restriction must have 'base' attribute or 'simpleType' child tag!",
                    restrictionNode)

            if restrictionNode.hasAttribute("base") and restrictionNode.getFirstChildNS(
                    self.inputNsURI,
                    "simpleType") != None:
                self._addError(
                    "Simple type restriction must not have 'base' attribute and 'simpleType' child tag!",
                    restrictionNode)

            if restrictionNode.hasAttribute("base"):
                self._checkType(restrictionNode, "base", self.xsdTypeDict)

            minExcl = restrictionNode.getFirstChildNS(
                self.inputNsURI,
                "minExclusive")
            minIncl = restrictionNode.getFirstChildNS(
                self.inputNsURI,
                "minInclusive")
            if minExcl is not None and minIncl is not None:
                self._addError(
                    "Restriction attributes 'minExclusive' and 'minInclusive' cannot be defined together!",
                    restrictionNode)
            maxExcl = restrictionNode.getFirstChildNS(
                self.inputNsURI,
                "maxExclusive")
            maxIncl = restrictionNode.getFirstChildNS(
                self.inputNsURI,
                "maxInclusive")
            if maxExcl is not None and maxIncl is not None:
                self._addError(
                    "Restriction attributes 'maxExclusive' and 'maxInclusive' cannot be defined together!",
                    restrictionNode)
        ...
        for restrictionNode in restrictionNodes:
            try:
                if restrictionNode.hasAttribute("base"):
                    facetNsName = self._getFacetType(
                        restrictionNode, [
                            restrictionNode.getParentNode(), ], self.xsdTypeDict)
                    if not facetNsName:
                        continue
                    if facetNsName in _localFacetDict:
                        suppFacets = _localFacetDict[facetNsName]
                    else:
                        suppFacets, dummy, dummy = self.xsdTypeDict[facetNsName].getXPathList(
                            ".//hfp:hasFacet/@name" %
                            vars())
                    ...
                    for childNode in restrictionNode.getChildren():
                        if childNode.getLocalName() in suppFacets:
                            if childNode.getLocalName() in specifiedFacets:
                                specifiedFacets[
                                    childNode.getLocalName()] = childNode["value"]
                            facetElementNode = self.xsdElementDict[
                                childNode.getNsName()]
                            try:
                                self._checkElementTag(
                                    facetElementNode, restrictionNode, (childNode,), 0)
                            except TagException as errInst:
                                self._addError(
                                    errInst.errstr,
                                    errInst.node,
                                    errInst.endTag)
                            if childNode.getLocalName() in (
                                    "enumeration",
                                    "minExclusive",
                                    "minInclusive",
                                    "maxExclusive",
                                    "maxInclusive"):
                                simpleTypeReturnDict = self._checkSimpleType(
                                    restrictionNode,
                                    "base",
                                    childNode,
                                    "value",
                                    ...
                                    None,
                                    checkAttribute=1)
                                if simpleTypeReturnDict is not None and "orderedValue" in simpleTypeReturnDict:
                                    if childNode.getLocalName(
                                    ) != "enumeration":
                                        specifiedFacets[
                                            childNode.getLocalName()] = simpleTypeReturnDict["orderedValue"]
                            ...
                            self._addError(
                                "Facet %s not allowed for base type %s!" %
                                (childNode.getLocalName(),
                                 repr(
                                    restrictionNode["base"])),
                                childNode)
                    if specifiedFacets["length"] != None:
                        if specifiedFacets["minLength"] != None or specifiedFacets[
                                "maxLength"] != None:
                            self._addError(
                                "Facet 'minLength' and 'maxLength' not allowed if facet 'length' is specified!",
                                restrictionNode)
                    else:
                        if specifiedFacets["maxLength"] != None and specifiedFacets[
                                "minLength"] != None:
                            if int(
                                    specifiedFacets["maxLength"]) < int(
                                    specifiedFacets["minLength"]):
                                self._addError(
                                    "Facet 'maxLength' < facet 'minLength'!",
                                    restrictionNode)

                    if specifiedFacets["totalDigits"] != None and specifiedFacets[
                            "fractionDigits"] != None:
                        if int(
                                specifiedFacets["totalDigits"]) < int(
                                specifiedFacets["fractionDigits"]):
                            self._addError(
                                "Facet 'totalDigits' must be >= 'fractionDigits'!",
                                restrictionNode)

                    if specifiedFacets["minExclusive"] != None and specifiedFacets[
                            "minInclusive"] != None:
                        self._addError(
                            "Facets 'minExclusive' and 'minInclusive' are mutually exclusive!",
                            restrictionNode)
                    if specifiedFacets["maxExclusive"] != None and specifiedFacets[
                            "maxInclusive"] != None:
                        self._addError(
                            "Facets 'maxExclusive' and 'maxInclusive' are mutually exclusive!",
                            restrictionNode)
                    ...
                    if minValue is not None and maxValue is not None and maxValue < minValue:
                        self._addError(
                            "maxValue facet < minValue facet!",
                            restrictionNode)

            except TagException:
                self._addError(
                    "Primitive type for base type not found!",
                    restrictionNode)

        listNodes, dummy, dummy = self.inputRoot.getXPathList(
            ".//%(prefix)slist" %
            vars())
        for listNode in listNodes:
            ...
            if not listNode.hasAttribute("itemType") and listNode.getFirstChildNS(
                    self.inputNsURI,
                    "simpleType") == None:
                self._addError(
                    "List type must have 'itemType' attribute or 'simpleType' child tag!",
                    listNode)
            elif listNode.hasAttribute("itemType") and listNode.getFirstChildNS(self.inputNsURI, "simpleType") != None:
                self._addError(
                    "List type must not have 'itemType' attribute and 'simpleType' child tag!",
                    listNode)
            elif listNode.hasAttribute("itemType"):
                itemType = self._checkType(
                    listNode,
                    "itemType",
                    self.xsdTypeDict)
                if itemType in self.xsdTypeDict:
                    if self.xsdTypeDict[
                            itemType].getLocalName() != "simpleType":
                        self._addError(
                            "ItemType %s must be a simple type!" %
                            (repr(itemType)),
                            listNode)
                    elif self.xsdTypeDict[itemType].getFirstChild().getLocalName() == "list":
                        self._addError(
                            "ItemType %s must not be a list type!" %
                            (repr(itemType)),
                            listNode)

        unionNodes, dummy, dummy = self.inputRoot.getXPathList(
            ".//%(prefix)ssimpleType/%(prefix)sunion" %
            vars())
            ...
            if not unionNode.hasAttribute("memberTypes"):
                ...
                for memberType in string.split(unionNode["memberTypes"]):
                    ...
                    if memberNsName in self.xsdTypeDict:
                        if self.xsdTypeDict[
                                memberNsName].getLocalName() != "simpleType":
                            self._addError(
    ...
    def _checkIdentityConstraintsSecondLevel(self):
        ...
        for identityConstraintNode in identityConstraintNodes:
            ...
            selectorNode = identityConstraintNode.getFirstChildNS(
                XSD_NAMESPACE,
                "selector")
            self._checkNodeId(selectorNode)
            try:
                completeChildList, attrNodeList, attrNsNameFirst = identityConstraintNode.getParentNode(
                ).getXPathList(selectorNode["xpath"], selectorNode)
                if attrNsNameFirst is not None:
                    self._addError(
                        "Selection of attributes is not allowed for selector!",
                        selectorNode)
            except Exception as errstr:
                self._addError(errstr, selectorNode)

            try:
                fieldNode = identityConstraintNode.getFirstChildNS(
                    XSD_NAMESPACE,
                    "field")
                identityConstraintNode.getParentNode().getXPathList(
                    fieldNode["xpath"],
                    fieldNode)
    ...
    def _checkFixedDefault(self, node):
        if node.hasAttribute("default") and node.hasAttribute("fixed"):
            self._addError(
                "%s may have 'default' OR 'fixed' attribute!" %
                repr(
                    node.getLocalName()),
                node)
    ...
    def _checkReference(self, node, dict):
        ...
        if baseNsName in dict:
            ...
            if fixedValue is not None and fixedRefValue is not None and fixedValue != fixedRefValue:
                self._addError(
                    "Fixed value %s of attribute does not match fixed value %s of reference!" %
                    (repr(fixedValue), repr(fixedRefValue)), node)
    ...
    def _checkBaseType(self, node, baseNsName, dict, typeNsName=None):
        if baseNsName not in dict and baseNsName != (
                XSD_NAMESPACE,
                "anySimpleType"):
            self._addError(
            ...
            if typeNsName == (XSD_NAMESPACE, "simpleContent"):
                if node.getNsName() == (XSD_NAMESPACE, "restriction"):
                    if (baseNsName != (XSD_NAMESPACE, "anySimpleType") and
                        ...
                        self._addError(
                            "Referred type %s must be a complex type with simple content!" %
                            (repr(baseNsName)), node)
                else:  # extension
                    if (
                        baseNsName == (
                            XSD_NAMESPACE,
                            "anySimpleType") or dict[baseNsName].getNsName() == (
                            XSD_NAMESPACE,
                            "simpleType") or (
                            dict[baseNsName].getNsName() == (
                                XSD_NAMESPACE,
                                "complexType") and dict[baseNsName].getFirstChild().getNsName() == typeNsName)):
                        pass
                    else:
                        self._addError(
                            "Referred type %s must be a simple type or a complex type with simple content!" %
                            (repr(baseNsName)),
                            node)
            else:
                if typeNsName == (
                        XSD_NAMESPACE,
                        "simpleType") and baseNsName == (
                        XSD_NAMESPACE,
                        "anySimpleType"):
                    pass
                elif dict[baseNsName].getNsName() != typeNsName:
                    self._addError(
                        "Referred type %s must be a %s!" %
                        (repr(baseNsName), repr(typeNsName)), node)

    def _checkKeyRef(self, keyrefNode, dict):
        baseNsName = keyrefNode.getQNameAttribute("refer")
        if baseNsName not in dict:
            self._addError(
                "keyref refers unknown key %s!" %
                (repr(baseNsName)),
                keyrefNode)
        else:
            keyNode = dict[baseNsName]["Node"]
            if keyNode.getNsName() not in (
                    (XSD_NAMESPACE, "key"), (XSD_NAMESPACE, "unique")):
                self._addError(
                    "reference to non-key constraint %s!" %
                    (repr(baseNsName)),
                    keyrefNode)
            if len(
                keyrefNode.getChildrenNS(
                    XSD_NAMESPACE,
                    "field")) != len(
                keyNode.getChildrenNS(
                    XSD_NAMESPACE,
                    "field")):
    ...
    def _checkNodeId(self, node, unambiguousPerFile=1):
        if node.hasAttribute("id"):
            ...
            if nodeId not in self.xsdIdDict:
                ...
                self._addError(
                    "There are multiple occurences of ID value %s!" %
                    repr(nodeId), node)
    ...
    def _getFacetType(self, node, parentNodeList, xsdTypeDict):
        ...
        try:
            ...
            self._addError(
                "Base type %s must be an atomic simple type definition or a builtin type!" %
                repr(baseNsName),
                node)
            return None
        ...
        if baseNode in parentNodeList:
            self._addError(
                "Circular type definition (type is contained in its own type hierarchy)!",
                node)
            return None
        ...
        if baseNode.getNsName() == (XSD_NAMESPACE, "simpleType"):
            if baseNode.getAttribute("facetType") != None:
                ...
                node.getParentNode()["facetType"] = node.nsName2QName(
                    facetType)
                return facetType
            else:
                for baseNodeType in ("list", "union"):
                    if baseNode.getFirstChildNS(
                            XSD_NAMESPACE,
                            baseNodeType) != None:
                        return (XSD_NAMESPACE, baseNodeType)
                else:
                    parentNodeList.append(node)
                    return self._getFacetType(
                        baseNode.getFirstChildNS(
                            XSD_NAMESPACE,
                            "restriction"),
                        parentNodeList,
                        xsdTypeDict)
