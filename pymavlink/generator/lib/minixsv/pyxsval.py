...
def parseAndValidateXmlInput(inputFile, xsdFile=None, validateSchema=0, **kw):
    ...
    return xsValidator.validateXmlInput(
        inputFile,
        inputTreeWrapper,
        xsdFile,
        validateSchema)
...
class XsValidator:
    ...
    def __init__(
        ...
        self.xmlIf = _xmlIfDict[xmlIfClass](
            verbose,
            useCaching,
            processXInclude)
    ...
    def validateXmlInput(
        ...
        xsdTreeWrapperList = self._readReferencedXsdFiles(
            inputTreeWrapper,
            validateSchema)
        if xsdTreeWrapperList == []:
            # if the input file doesn't contain schema references => use given
            # xsdFile
    ...
    def _readReferencedXsdFiles(self, inputTreeWrapper, validateSchema):
        xsdTreeWrapperList = []
        # a schemaLocation attribute is expected in the root tag of the XML
        # input file
        xsdFileList = self._retrieveReferencedXsdFiles(inputTreeWrapper)
        for namespace, xsdFile in xsdFileList:
            try:
                xsdTreeWrapper = self.parse(
                    xsdFile,
                    inputTreeWrapper.getRootNode().getAbsUrl())
            except IOError as e:
                if e.errno == 2:  # catch IOError: No such file or directory
                    self.errorHandler.raiseError(
                        "XML schema file %s not found!" %
                        (xsdFile),
                        inputTreeWrapper.getRootNode())
                else:
            ...
            if namespace != xsdTreeWrapper.getRootNode().getAttributeOrDefault(
                    "targetNamespace",
                    None):
                self.errorHandler.raiseError(
                    "Namespace of 'schemaLocation' attribute doesn't match target namespace of %s!" %
                    (xsdFile),
                    inputTreeWrapper.getRootNode())
    ...
    def _retrieveReferencedXsdFiles(self, inputTreeWrapper):
        ...
        if inputRootNode.hasAttribute((XSI_NAMESPACE, "schemaLocation")):
            attributeValue = inputRootNode.getAttribute(
                (XSI_NAMESPACE, "schemaLocation"))
            ...
            if len(attrValList) % 2 == 0:
                ...
                self.errorHandler.raiseError(
                    "'schemaLocation' attribute must have even number of URIs (pairs of namespace and xsdFile)!")

        if inputRootNode.hasAttribute(
            (XSI_NAMESPACE,
             "noNamespaceSchemaLocation")):
            attributeValue = inputRootNode.getAttribute(
                (XSI_NAMESPACE, "noNamespaceSchemaLocation"))
            attrValList = string.split(attributeValue)
...
def _interfaceFactoryMinidom(verbose, useCaching, processXInclude):
    return genxmlif.chooseXmlIf(
        genxmlif.XMLIF_MINIDOM,
        verbose,
        useCaching,
        processXInclude)
