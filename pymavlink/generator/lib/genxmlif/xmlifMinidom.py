...
class XmlInterfaceMinidom (XmlInterfaceDom):
    ...
    def createXmlTree(
        ...
        treeWrapper = self.treeWrapperClass(
            self,
            InternalMinidomTreeWrapper(domTree),
            self.useCaching)

        intRootNodeWrapper = InternalMinidomElementWrapper(
    ...
    def parse(self, file, baseUrl="", internalOwnerDoc=None):
        ...
        try:
            ...
            if self.processXInclude:
                ...
                self.xInclude(
                    builder.treeWrapper.getRootNode(),
                    absUrl,
                    internalOwnerDoc)
...
class ExtExpatBuilderNS (ExpatBuilderNS, XmlIfBuilderExtensionDom):
    ...
    def __init__(self, filePath, absUrl, xmlIf):
        ...
        self.treeWrapper = xmlIf.treeWrapperClass(
            self,
            internalMinidomTreeWrapper,
            xmlIf.useCaching)
        ...
        self.getParser(
        ).EndNamespaceDeclHandler = self.end_namespace_decl_handler
        self.curNamespaces = []
...
#                internalMinidomElementWrapper.xmlIfExtElementWrapper.setAttribute((XMLNS_NAMESPACE, namespace[0]), namespace[1])
    ...
    def end_element_handler(self, name):
        XmlIfBuilderExtensionDom.endElementHandler(
            self,
            self.curNode.xmlIfExtInternalWrapper,
            self.getParser().ErrorLineNumber)
    ...
    def end_namespace_decl_handler(self, prefix):
        assert self.curNamespaces.pop(
            0)[0] == prefix, "implementation confused"
