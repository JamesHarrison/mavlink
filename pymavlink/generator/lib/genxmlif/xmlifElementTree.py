...
# from version 2.5 on the elementtree module is part of the standard
# python distribution
...
class XmlInterfaceElementTree (XmlInterfaceBase):
    ...
    def createXmlTree(
            ...
            publicId=None,
            systemId=None):
        rootNode = ElementExtension(
            toClarkQName(xmlRootTagName),
            attributeDict)
        rootNode.xmlIfExtSetParentNode(None)
        treeWrapper = self.treeWrapperClass(
            self,
...
class ElementExtension (_ElementInterface):
    ...
    def xmlIfExtProcessWsElementValue(self, wsAction):
        noOfTextFragments = reduce(lambda sum,
                                   child: sum + (child.tail is not None),
                                   self.getchildren(),
                                   0)
        noOfTextFragments += (self.text is not None)
        ...
        if self.text is not None:
            if noOfTextFragments == 1:
                rstrip = 1
            self.text = processWhitespaceAction(
                self.text,
                wsAction,
                lstrip,
                rstrip)
        ...
        for child in self.getchildren():
            if child.tail is not None:
                if noOfTextFragments == 1:
                    rstrip = 1
                child.tail = processWhitespaceAction(
                    child.tail,
                    wsAction,
                    lstrip,
                    rstrip)
...
class ExtXMLTreeBuilder (XMLTreeBuilder, XmlIfBuilderExtensionBase):
    ...
    def start(self, element, attributes):
        ...
        XmlIfBuilderExtensionBase.startElementHandler(
            self,
            element,
        ...
        if len(self._target._elem) > 1:
            ...
            for namespace in self.namespaces:
                if namespace[1] != None:
                    element.xmlIfExtElementWrapper.setAttribute(
                        (XMLNS_NAMESPACE, namespace[0]), namespace[1])
