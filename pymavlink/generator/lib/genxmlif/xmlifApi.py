...
class XmlTreeWrapper:
    ...
    def createElement(self, tupleOrLocalName, attributeDict=None, curNs=[]):
        ...
        elementNode = self.__tree.xmlIfExtCreateElement(
            nsName,
            attributeDict,
            curNs)
    ...
    def cloneTree(self):
        ...
        treeWrapperCopy = self.__class__(
            self.xmlIf,
            self.__tree.xmlIfExtCloneTree(
                rootElementWrapperCopy.element),
            self.__useCaching)
    ...
    def printTree(self, prettyPrint=0, printElementValue=1, encoding=None):
        ...
        if encoding != "utf-8" and encoding != "us-ascii":
            ...
            text = ""
        return text + self.getRootNode().printNode(deep=1,
                                                   prettyPrint=prettyPrint,
                                                   printElementValue=printElementValue,
                                                   encoding=encoding)
...
class XmlElementWrapper:
    ...
    def cloneNode(self, deep, cloneCallback=None):
        ...
        elementWrapperCopy = self.__class__(
            elementCopy,
            self.treeWrapper,
            initAttrSeq=0)
        ...
        if deep:
            for childElement in self.element.xmlIfExtGetChildren():
                childWrapperElementCopy = childElement.xmlIfExtElementWrapper.cloneNode(
                    deep,
                    cloneCallback)
                childWrapperElementCopy.element.xmlIfExtSetParentNode(
                    elementWrapperCopy.element)
                elementWrapperCopy.element.xmlIfExtAppendChild(
                    childWrapperElementCopy.element)
...
#++++++++++++ methods concerning the tag name ++++++++++++++++++++++++
    ...
    def getNsName(self):
        ...
        try:
            ...
            nsName = NsNameTupleFactory(
                (self.getNamespaceURI(), self.getLocalName()))
            if self.__useCaching():
...
%(indent)s</%(qName)s>%(tailText)s%(lf)s'''
            ...
            for childNode in self.getChildren():
                subTreeStringList.append(
                    childNode.printNode(
                        ...
                        childAttrMaxLengthDict,
                        printElementValue))
            tailText = escapeCdata(
                self.element.xmlIfExtGetElementTailText(),
                encoding)
        ...
        for attrName in self.getAttributeList():
            ...
            attributeStringList.append(
                ' %s="%s"%*s' %
                (qName, attrValue, align - len(attrValue), ""))
        attributeString = string.join(attributeStringList, "")

        qName = self.getQName()
        if printElementValue:
            if deep:
                elementValueString = escapeCdata(
                    self.element.xmlIfExtGetElementText(),
                    encoding)
            else:
                elementValueString = escapeCdata(
                    self.getElementValue(
                        ignoreEmtpyStringFragments=1),
                    encoding)
        else:
            elementValueString = ""
...
#++++++++++++ methods concerning the children of the current node ++++++++
    ...
    def getChildren(self, tagFilter=None):
        ...
        if tagFilter in (None, '*', (None, '*')):
            ...
            # handle (namespace, '*')
            children = [child for child in self.element.xmlIfExtGetChildren(
            ) if child.xmlIfExtElementWrapper.getNamespaceURI() == tagFilter[0]]
        else:
    ...
    def getFirstChildWithKey(
        ...
        children = self.getChildren(tagFilter)
        childrenWithKey = [
            child for child in children if child[keyAttr] == keyValue]
        if childrenWithKey != []:
    ...
    def getElementsByTagName(self, tagFilter=None):
        ...
        if tagFilter in (None, '*', (None, '*'), (None, None)):
            ...
            # handle (namespace, '*')
            descendants = [desc for desc in self.element.xmlIfExtGetElementsByTagName(
            ) if desc.xmlIfExtElementWrapper.getNamespaceURI() == tagFilter[0]]
        else:
            nsNameFilter = NsNameTupleFactory(tagFilter)
            descendants = self.element.xmlIfExtGetElementsByTagName(
                nsNameFilter)
        ...
        return [
            descendant.xmlIfExtElementWrapper for descendant in descendants]
    ...
    def getIterator(self, tagFilter=None):
        ...
        if tagFilter in (None, '*', (None, '*'), (None, None)):
            ...
            # handle (namespace, '*')
            matchingElements = [desc for desc in self.element.xmlIfExtGetIterator(
            ) if desc.xmlIfExtElementWrapper.getNamespaceURI() == tagFilter[0]]
        else:
            nsNameFilter = NsNameTupleFactory(tagFilter)
    ...
    def appendChild(self, tupleOrLocalNameOrElement, attributeDict={}):
        ...
        if not isinstance(tupleOrLocalNameOrElement, self.__class__):
            childElementWrapper = self.__createElement(
    ...
    def insertBefore(
        ...
        if refChild is None:
            ...
            self.element.xmlIfExtInsertBefore(
                childElementWrapper.element,
                refChild.element)
            self.__clearChildrenCache(childElementWrapper.getNsName())
        return childElementWrapper
    ...
    def insertSubtree(
        ...
        """
        if refChildWrapper is not None:
            self.element.xmlIfExtInsertSubtree(
                refChildWrapper.element,
                subTreeWrapper.getTree(),
                insertSubTreeRootNode)
        else:
            self.element.xmlIfExtInsertSubtree(
                None,
                subTreeWrapper.getTree(),
...
#++++++++++++ methods concerning the attributes of the current node ++++++
    ...
    def setAttribute(self, tupleOrAttrName, attributeValue):
        ...
        if not isinstance(attributeValue, StringTypes):
            raise TypeError(
                "%s (attribute %s) must be a string!" %
                (repr(attributeValue), repr(tupleOrAttrName)))
        ...
        self.element.xmlIfExtSetAttribute(
            nsNameAttrName,
            attributeValue,
            self.getCurrentNamespaces())
...
#++++++++++++ methods concerning the content of the current node +++++++++
    ...
    def getElementValue(self, ignoreEmtpyStringFragments=0):
        ...
        """
        return "".join(
            self.getElementValueFragments(ignoreEmtpyStringFragments))
    ...
    def getElementValueFragments(self, ignoreEmtpyStringFragments=0):
        ...
        Method has to be implemented by derived classes!
        """
        return self.element.xmlIfExtGetElementValueFragments(
...
#++++++++++++ methods concerning the info about the current node in the XM
        ...
        if qName is not None:
            ...
            for prefix, namespaceURI in self.getCurrentNamespaces():
                ...
                if qNamePrefix is None:
                    ...
                    raise ValueError(
                        "Namespace prefix '%s' not bound to a namespace!" %
                        (qNamePrefix))
        else:
            nsName = (None, None)
        ...
        if qName is not None:
            ...
            for prefix, namespaceURI in self.getCurrentNamespaces():
                ...
                if qNamePrefix is None:
                    ...
                    raise LookupError(
                        "Namespace for QName '%s' not found!" %
                        (qName))
        else:
            namespace = EMPTY_NAMESPACE
        return namespace
    ...
    def getNsPrefix(self, nsLocalName):
        ...
        for prefix, namespace in self.getCurrentNamespaces():
            ...
            if ns is None:
                ...
                raise LookupError(
                    "Prefix for namespaceURI '%s' not found!" %
                    (ns))
...
#++++++++++++ limited XPath support ++++++++++++++++++++
                ...
                if localStep == "":
                    ...
                    for childNode in childList:
                        if attrName == '*':
                            ...
                            attrNsName = namespaceRef.qName2NsName(
                                attrName,
                                useDefaultNs=0)
                            if attrNsName[1] == '*':
                                for attr in list(
                                        childNode.getAttributeDict().keys()):
                                    if attr[0] == attrNsName[0]:
                                        if attrNodeList == []:
                                            attrNsNameFirst = attrNsName
                                        attrNodeList.append(childNode)
                                        stepChildList.append(
                                            childNode.getAttribute(attr))
                            elif childNode.hasAttribute(attrNsName):
                                if attrNodeList == []:
                                    attrNsNameFirst = attrNsName
                                attrNodeList.append(childNode)
                                stepChildList.append(
                                    childNode.getAttribute(attrNsName))
                    childList = stepChildList
                else:
                    nsLocalName = namespaceRef.qName2NsName(
                        localStep,
                        useDefaultNs=useDefaultNs)
                    if descendantOrSelf:
                        descendantOrSelf = 0
                        if localStep == "*":
                        ...
                        for childNode in childList:
                            if localStep == "*":
                                ...
                                stepChildList.extend(
                                    childNode.getChildrenNS(
                                        nsLocalName[0],
                                        nsLocalName[1]))
                    childList = stepChildList
    ...
    def __createElement(self, tupleOrLocalName, attributeDict):
        ...
        """
        childElementWrapper = self.treeWrapper.createElement(
            tupleOrLocalName,
            attributeDict,
            self.curNs[:])  # TODO: when to be adapted???)
    ...
    def __updateAttrMaxLengthDict(self, attrMaxLengthDict):
        ...
        for attrName, attrValue in list(self.getAttributeDict().items()):
            ...
            if attrName not in attrMaxLengthDict:
                ...
                attrMaxLengthDict[attrName] = max(
                    attrMaxLengthDict[attrName],
                    attrLength)
