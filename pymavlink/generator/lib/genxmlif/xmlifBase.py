...
class XmlIfBuilderExtensionBase:
    ...
    def startElementHandler(
        ...
        elementWrapper = self.elementWrapperClass(
            curNode,
            self.treeWrapper,
            curNs,
            initAttrSeq=0)
        ...
        if attributes != []:
            ...
            attrList = sorted(elementWrapper.getAttributeDict().keys())
    ...
    def __getBaseUrl(self, elementWrapper):
        ...
        if elementWrapper.hasAttribute(nsNameBaseAttr):
            return convertToAbsUrl(
                elementWrapper.getAttribute(nsNameBaseAttr),
                self.baseUrlStack[0])
        else:
