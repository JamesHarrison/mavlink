    ...
    def xInclude(self, elementWrapper, baseUrl, ownerDoc):
        ...
        for childElementWrapper in elementWrapper.getChildren():
            ...
            if childElementWrapper.getNsName() == (XINC_NAMESPACE, "include"):
                ...
                parse = childElementWrapper.getAttributeOrDefault(
                    "parse",
                    "xml")
                ...
                try:
                    if parse == "xml":
                        ...
                        elementWrapper.replaceChildBySubtree(
                            childElementWrapper,
                            subTreeWrapper)
                    elif parse == "text":
                        ...
                        elementWrapper.element.element.insertBefore(
                            newTextNode,
                            childElementWrapper.element.element)
                        elementWrapper.removeChild(childElementWrapper)
                        fp.close()
                    else:
                        raise GenXmlIfError(
                            "%s: line %s: XIncludeError: Invalid 'parse' Attribut: '%s'" %
                            (filePath, line, parse))
                except IOError as errInst:
                    raise GenXmlIfError(
                        "%s: line %s: IOError: %s" %
                        (filePath, line, str(errInst)))
            elif childElementWrapper.getNsName() == (XINC_NAMESPACE, "fallback"):
                raise GenXmlIfError(
                    "%s: line %s: XIncludeError: xi:fallback tag must be child of xi:include" %
                    (filePath, line))
            else:
...
class InternalDomElementWrapper:
    ...
    def xmlIfExtCloneNode(self):
        nodeCopy = self.__class__(
            self.element.cloneNode(
                deep=0),
            self.internalDomTreeWrapper)
...
#            nodeCopy.xmlIfExtSetAttribute(nsAttrName, attrValue, self.xmlIfExtElementWrapper.getCurrentNamespaces())
    ...
    def xmlIfExtGetElementsByTagName(self, tagFilter=('*', '*')):
        elementList = self.element.getElementsByTagNameNS(
            tagFilter[0],
            tagFilter[1])
    ...
    def xmlIfExtGetIterator(self, tagFilter=('*', '*')):
        ...
        elementList.extend(
            self.element.getElementsByTagNameNS(
                tagFilter[0],
                tagFilter[1]))
        return [element.xmlIfExtInternalWrapper for element in elementList]
    ...
    def xmlIfExtInsertSubtree(
        ...
        for childElement in childElementList:
            if refChildElement is not None:
                self.element.insertBefore(
                    childElement.element,
                    refChildElement.element)
            else:
    ...
    def xmlIfExtGetAttributeDict(self):
        attribDict = {}
        for nsAttrName, attrNodeOrValue in list(
    ...
    def xmlIfExtGetElementValueFragments(self, ignoreEmtpyStringFragments):
        ...
        if ignoreEmtpyStringFragments:
            elementValueList = [
                s for s in elementValueList if collapseString(s) != ""]
        if elementValueList == []:
            elementValueList = ["", ]
    ...
    def xmlIfExtSetElementValue(self, elementValue):
        if self.__xmlIfExtGetChildTextNodes() == []:
            textNode = self.internalDomTreeWrapper.xmlIfExtCreateTextNode(
                elementValue)
    ...
    def xmlIfExtProcessWsElementValue(self, wsAction):
        ...
        if len(textNodes) == 1:
            textNodes[0].data = processWhitespaceAction(
                textNodes[0].data,
                wsAction)
            ...
            for textNode in textNodes[1:-1]:
                textNode.data = processWhitespaceAction(
                    textNode.data,
                    wsAction,
                    lstrip,
                    rstrip=0)
                if len(textNode.data) > 0 and textNode.data[-1] == " ":
                    ...
                    lstrip = 0
            textNodes[-
                      1].data = processWhitespaceAction(textNodes[-
                                                                  1].data, wsAction, lstrip)
    ...
    def __xmlIfExtGetChildTextNodes(self):
        ...
        return [
            e for e in self.element.childNodes if (
                e.nodeType in (
                    Node.TEXT_NODE,
                    Node.CDATA_SECTION_NODE))]                         # from element's child nodes
