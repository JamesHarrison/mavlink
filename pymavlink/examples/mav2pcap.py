...
# packet length information and the next SOF magic. Additionally the CRC
# is verified.

# Hint: A MAVLink protocol dissector (parser) for Wireshark may be
# generated by mavgen.py.

# dependency: Python construct library (python-construct on
# Debian/Ubuntu), "easy_install construct" elsewhere
...
# copied from ardupilotmega.h (git changeset
# 694536afb882068f50da1fc296944087aa207f9f, Dec 02 2012
...
class pcap:
    ...
    def __init__(self, stream, mode='rb', snaplen=65535, linktype=1):
        ...
        if hdr:
            ...
            (self.magic,
             ...
             self.sigfigs,
             self.snaplen,
             self.linktype) = struct.unpack(self._endian + 'IHHIIII',
                                            hdr)
            if (version_major, version_minor) != (2, 4):
                raise IOError(
                    'Cannot handle file version %d.%d' %
    ...
    def read(self):
        ...
        if not hdr:
            return
        (tv_sec, tv_usec, caplen, length) = struct.unpack(
...
def convert_file(mavlink_file, pcap_file):
    ...
    while not done:
        ...
        try:
            ...
            if chr(MAVLINK_MAGIC) != next_magic:
                # damn, retry
                print(
                    ("packet %d has invalid length, crc error: %d" % (i, crc_flag)))

                # skip one char to look for a new SOF next round, stow away
                # skipped char
                skipped_char = data[0]
                data = data[1:]
                continue
    ...
    print(("converted %d valid packets, %d crc errors, %d junk fragments (total %f%% of junk)" % (
        cnt_ok, cnt_crc, cnt_junk, 100. * float(cnt_junk + cnt_crc) / (cnt_junk + cnt_ok + cnt_crc))))
...
pcap_file = pcap(
    ...
    args.output_file.mode,
    linktype=147)  # special trick: linktype USER0
