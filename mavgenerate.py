...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.out_button = Button(
            self,
            text="Browse",
            command=self.browseOutDirectory)
        ...
        self.validate_button = Checkbutton(
            self,
            variable=self.validate_value,
            onvalue=True,
            offvalue=False)
    ...
    def browseXMLFile(self):
        ...
        xml_file = tkinter.filedialog.askopenfilename(
            parent=self,
            title='Choose a definition file')
    ...
    def generateHeaders(self):
        ...
        if os.path.isdir(self.out_value.get()):
            if not tkinter.messagebox.askokcancel(
                'Overwrite Headers?',
                'The output directory \'{0}\' already exists. Headers may be overwritten if they already exist.'.format(
                    self.out_value.get())):
                return
...
def formatErrorMessage(message):
    ...
    prefix = ("An error occurred in mavgen:" if len(
        matches) == 1 else "Errors occurred in mavgen:\n")
